...
export default new Event("messageCreate", (message) => {
  ...
  const cmd = message.client.commands.find(
    (x) =>
      ...
      (x.options.aliases &&
        x.options.aliases.includes(commandName.toLowerCase()))
  );
  if (!cmd) return;

  if (cmd.options.groupOnly && message.chatID !== process.env.chatID) return;
  if (
    cmd.options.allowed_users &&
    !cmd.options.allowed_users.includes(message.authorID)
  )
    return;
  if (
    cmd.options.denied_users &&
    cmd.options.denied_users.includes(message.authorID)
  )
    return;
  ...
  run.catch((error) => {
    console.log(
      `Erreur lors de l'exécution de la commande ${cmd.options.name}`
    );
    console.log(error);
  });
});
